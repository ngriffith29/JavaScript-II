...
let cFirstNames = () => {
    runners.map(function(firstName){
       ...
       return allCaps.push(firstName.first_name.toUpperCase())
...
let largeShirts = [];
const LGS = runners.filter(function (shirt){
    if (shirt.shirt_size === "L"){
       return largeShirts.push(`${shirt.first_name} ${shirt.last_name} Due to an ordering issue you need to select a shirt size other than LG`)
    }
})
...
let sum = []
const seperateNums =  runners.forEach(function(donate){
             
             sum.push(donate.donation)

                   
    return ticketPriceTotal = sum.reduce(function(a,b){
        return a+b;
    })
